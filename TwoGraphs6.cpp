...
And draw graph with some intergration process
*/
...
void TwoGraphs6()
{
    ...
    counter->open("./data/mcounter_29.csv",ios::in);
    thinq->open("./data/puri_29.csv", ios::in);
    ...
    while(!thinq->eof())
    {
        ...
        if(num==0)
        {
            order1=stod(str_buf2);
            graph2->SetPoint(num,time,stod(str_buf2));
            cout<<num<<","<<str_buf2<<endl;
        }
        if(num==1)
        {
            order2=stod(str_buf2);
            graph2->SetPoint(num,time,stod(str_buf2));
            cout<<num<<","<<str_buf2<<endl;
        }
        else
        {
            ...
            graph2->SetPoint(num,time,(stod(str_buf2)+order1+order2)/3);
            cout<<num<<","<<(stod(str_buf2)+order1+order2)/3<<endl;
            order2=order1;
            order1=stod(str_buf2);
        }
        // if(time>600&&time<700) continue;
    ...
    graph2->GetYaxis()->SetLimits(0,50);
